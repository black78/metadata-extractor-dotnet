...
namespace MetadataExtractor.Tests.Formats.Adobe
{
    ...
    {
        [NotNull]
        public static async Task<AdobeJpegDirectory> ProcessBytesAsync([NotNull] string filePath)
        {
            using (var stream = await(await TestHelper.OpenTestFileAsync(filePath).ConfigureAwait(false)).OpenStreamForReadAsync().ConfigureAwait(false))
                return new AdobeJpegReader().Extract(new SequentialStreamReader(stream));
        }
        ...
        [Fact]
        public async Task TestReadAdobeJpegMetadata1()
        {
            var directory = await ProcessBytesAsync("adobeJpeg1.jpg.appe");
