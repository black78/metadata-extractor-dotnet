...
namespace MetadataExtractor.Tests.Formats.Gif
{
    ...
    {
        [NotNull]
        public static async Task<GifHeaderDirectory> ProcessBytesAsync([NotNull] string file)
        {
            using (var stream = await(await TestHelper.OpenTestFileAsync(file).ConfigureAwait(false)).OpenStreamForReadAsync().ConfigureAwait(false))
                return new GifReader().Extract(new SequentialStreamReader(stream));
        }

        [Fact]
        public async Task TestMsPaintGif()
        {
            var directory = await ProcessBytesAsync("mspaint-10x10.gif");
        ...
        [Fact]
        public async Task TestPhotoshopGif()
        {
            var directory = await ProcessBytesAsync("photoshop-8x12-32colors-alpha.gif");
            Assert.False(directory.HasError);
            Assert.Equal("89a", directory.GetString(GifHeaderDirectory.TagGifFormatVersion));
